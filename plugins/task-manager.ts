...
function backup(): void {
  ...
  copyFileSync(DB_PATH, join(BACKUP_DIR, `task-manager-${ts}.db`));
  const backups = readdirSync(BACKUP_DIR)
    .filter((f) => f.startsWith("task-manager-"))
    .sort();
...
function renderTodoMd(database: Database, projectPath: string): string {
  ...
  const lines: string[] = [];
  lines.push("<!-- Auto-generated by task-manager plugin. Edits will be synced on next tool call. -->");
  lines.push("");
  lines.push("# TODO");
  lines.push("");
